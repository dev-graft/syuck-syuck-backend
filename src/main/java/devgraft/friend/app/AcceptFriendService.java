...
public class AcceptFriendService {
    ...
    public void acceptFriend(final String memberId, final Long friendRelationId) {
        ...
        if (friendRelation.compareSenderTo(memberId)) throw new SelfAcceptFriendException();
        // receiver가 memberId인지 검사 & 아닐 경우 예외처리
        if (!friendRelation.compareReceiverTo(memberId)) throw new UnrelatedAcceptFriendException();
