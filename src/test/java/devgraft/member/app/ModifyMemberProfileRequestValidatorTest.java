...
class ModifyMemberProfileRequestValidatorTest {
    private static final ModifyMemberProfileRequestValidator validator = new ModifyMemberProfileRequestValidator();

    @DisplayName("회원 프로필 수정 입력 값이 정규식에 일치하지 않으면 에러")
    @Test
    void validatedHasError() {
        final List<ValidationError> errors = validator.validate(ModifyMemberProfileRequest.builder()
                .nickname("!@#")
                .build());
